...
def main():
    ...
    if command == "decode":
        ...
        if sys.argv[2] != "-o":
            raise ValueError("Usage: download -o <output_file> <torrent_file>")
        output_file = sys.argv[3]
        torrent_file = sys.argv[4]
        ...
        with socket.create_connection((peer_ip, peer_port)) as s:
            ...
            # 3. Handle initial state (Wait for Bitfield, Send Interested, Wait for Unchoke)
            
            # Consume initial messages until unchoke
            # Note: We might get Bitfield, then Unchoke. Or just Unchoke.
            # We MUST send Interested first.
            
            # Read whatever message comes first (likely Bitfield)
            # But according to spec, we can send Interested immediately after handshake.
            
            # Let's peek/read messages.
            # A common pattern is:
            # <Recv Handshake>
            # <Recv Bitfield> (Optional)
            # <Send Interested>
            # <Recv Unchoke>
            
            # However, the previous `download_piece` implementation consumed one message (assumed Bitfield), 
            # then sent Interested, then waited for Unchoke.
            # Let's try to be slightly more robust or stick to what worked.
            
            # Read first message (Bitfield usually)
            msg_len_bytes = s.recv(4)
            while len(msg_len_bytes) < 4:
                 chunk = s.recv(4 - len(msg_len_bytes))
                 if not chunk: break
            ...
            if len(msg_len_bytes) == 4:
                ...
                if msg_id == 5: # Bitfield
                    ...
                    # If it's not Bitfield, it might be Unchoke already?
                    # Or Have?
                    # Since we consumed the header, we need to handle this message.
                    # But for now, let's assume the previous flow: Bitfield -> Interested -> Unchoke
                    # If this message was NOT Bitfield, we might have issues if we assume next is Unchoke.
                    # But let's proceed with sending Interested.
                    pass
            ...
            with open(output_file, "wb") as f:
                for piece_index in range(total_pieces):
                    ...
                    calculated_hash = hashlib.sha1(piece_data).digest()
                    if calculated_hash != expected_hash:
                         raise Exception(f"Piece {piece_index} integrity check failed")

                    f.write(piece_data)
                    print(f"Downloaded piece {piece_index}/{total_pieces}")
